...
const page = () => {
  ...
  return (
    <div className={`${spaceGrotesk.className} flex min-h-screen items-center justify-center gap-10 bg-gray-200 py-0 md:px-8 px-1 md:flex-row flex-col`}>
    {participants&&<div className="flex flex-col items-start justify-start p-8">
      {/* <span className="text-7xl font-bold font-gray-600">Kodessphere 2.0</span> */}
      <img src="/kodespherelogo.png" alt="kdsphr" className="w-full h-[100px]" />
      ...
      <div className="flex h-full w-fit md:flex-row flex-col items-start justify-end gap-5">
        {participants && participants?.map((participant)=>(<div key={participant.UserId} className="md:translate-y-[-100px] w-[380px] md:w-[280px] relative pointer-events-none select-none p-0">
            ...
            <p className="font-poppins absolute top-[83%] text-lg text-black font-bold left-[5%]">{participant.Team.TeamName}</p>
          </div>))}
      </div>
    {participants===null && !loading && <span className="self-center">
        Team Details Not Found
      </span>}

    {loading && <span className="self-center justify-self-center">
        Loading ID Cards...
      </span>}
    </div>
  );
