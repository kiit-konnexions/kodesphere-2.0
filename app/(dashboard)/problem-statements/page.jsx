...
const DomainBadge = ({ domain, darkMode }) => {
  return (
      <div className={`
...
const ProblemStatementDetail = ({ problem, darkMode }) => {
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          {imageSource ? (
              <div className="w-full md:w-1/3 flex justify-center items-start">
                <div className={`border ${darkMode ? 'border-zinc-600' : 'border-gray-300'} rounded-none overflow-hidden`}>
                  <img
                      src={imageSource}
                        ...
                        e.target.parentElement.innerHTML = `
                                        <div class="relative w-full h-full aspect-square">
                                            <div class="absolute inset-0 flex items-center justify-center">
                                                <div class="w-full h-0.5 ${darkMode ? 'bg-zinc-500' : 'bg-gray-400'} transform rotate-45"></div>
                                                <div class="w-full h-0.5 ${darkMode ? 'bg-zinc-500' : 'bg-gray-400'} transform -rotate-45"></div>
                                            </div>
                                        </div>
                                    `;
              ...
              <div className="w-full md:w-1/3 flex justify-center items-start">
                <div className={`border ${darkMode ? 'border-zinc-600' : 'border-gray-300'} aspect-square w-full max-w-xs`}>
                  <div className="relative w-full h-full">
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className={`w-full h-0.5 ${darkMode ? 'bg-zinc-500' : 'bg-gray-400'} transform rotate-45`}></div>
                      <div className={`w-full h-0.5 ${darkMode ? 'bg-zinc-500' : 'bg-gray-400'} transform -rotate-45`}></div>
                    </div>
                  </div>
                </div>
      ...
      <div className={`flex min-h-screen ${darkMode ? 'bg-zinc-800 text-white' : 'bg-white text-black'}`}>
        ...
        <main className={`flex-1 p-6 md:p-12 ${darkMode ? 'bg-zinc-800' : 'bg-white'} relative z-10 mt-16 sm:mt-0 mb-8`}>
          ...
          <div className={`transform transition-all duration-500 delay-200 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
            <DomainBadge domain={currentProblem.domain} darkMode={darkMode} />
          </div>
          ...
          <div className={`transform transition-all duration-500 delay-300 ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
            <ProblemStatementDetail problem={currentProblem} darkMode={darkMode} />
          </div>
        </main>
      </div>
  );
