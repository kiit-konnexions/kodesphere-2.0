  ...
  return (
      <div className={`relative w-full max-w-4xl mx-auto ${isLocked ? 'select-none' : ''}`}>
        ...
        <div className={`
          ...
          p-8
          rounded-none 
          ${darkMode ? 'bg-zinc-800/90' : 'bg-white/90'} 
...
const DownloadButton = ({isLocked, handleDownload, darkMode}) => {
  return (
      <div className="mt-8 text-center">
        <button
            ...
            className={`
          ${darkMode
                ? isLocked ? 'bg-zinc-700 text-zinc-400' : 'bg-green-600 hover:bg-green-700'
                : isLocked ? 'bg-gray-200 text-gray-400' : 'bg-green-600 hover:bg-green-700 text-white'
            }
          px-8 py-4
          rounded-none
          border ${darkMode ? 'border-zinc-600' : isLocked ? 'border-gray-300' : 'border-green-600'}
          font-bold
          transition-colors
          duration-300
          flex
        ...
        {isLocked && (
            <p className={`mt-3 text-sm ${darkMode ? 'text-zinc-400' : 'text-gray-500'}`}>
...
function CertificatePage() {
  ...
  return (
      <div className={`flex min-h-screen ${darkMode ? 'bg-zinc-800 text-white' : 'bg-white text-black'}`}>
        ...
        <main
            className={`flex-1 p-6 md:p-12 ${darkMode ? 'bg-zinc-800' : 'bg-white'} relative z-10 mt-16 sm:mt-0 mb-8`}>
