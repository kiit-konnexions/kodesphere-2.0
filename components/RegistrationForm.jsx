...
export default function RegistrationForm({setIsRegistered, setTid}) {
    ...
    useEffect(() => {
        if (session?.user) {
            setMembers((members) =>
                members.map((member) => (member.id === 1 ? {
                    ...member,
                    name: session.user.name || "",
                    email: session.user.email || ""
                } : member))
    ...
    const updateMember = (id, field, value) => {
        ...
        // If updating a phone field, check for duplicates immediately
        if (field === 'phone' && value.trim()) {
            // Wait for state update to complete
            setTimeout(() => {
                const updatedMembers = members.map(m =>
                    m.id === id ? {...m, phone: value} : m
                );
                const duplicates = updatedMembers.filter(m =>
                    m.phone === value && m.phone.trim()
                );

                if (duplicates.length > 1) {
                    setErrors(prev => ({
                        ...prev,
                        [`member-${id}-phone`]: "Phone number must be unique for each member"
                    }));
                }
            }, 0);
        }
    ...
    const validatePhoneNumber = (id, phone) => {
        if (!phone?.trim()) {
            setErrors(prev => ({...prev, [`member-${id}-phone`]: "Phone number is required"}));
            return false;
        } else if (!/^[0-9]{10}$/.test(phone)) {
            setErrors(prev => ({...prev, [`member-${id}-phone`]: "Must be a valid 10-digit phone number"}));
            return false;
        }

        // Only check for duplicate phone numbers within the form
        const phoneOccurrences = members.filter(m => m.phone === phone).length;
        if (phoneOccurrences > 1) {
            setErrors(prev => ({...prev, [`member-${id}-phone`]: "Phone number must be unique for each member"}));
            return false;
        }
    ...
    const validateForm = () => {
        ...
        members.forEach((member) => {
            ...
            // Phone validation including uniqueness check
            const phoneValid = validatePhoneNumber(member.id, member.phone);
            if (!phoneValid) {
                isValid = false;
            }
        });
